                    ...
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path, 'a') as f:
                                # f.write(('%g ' * 10 + '\n') % (frame_idx + 1, id, bbox_left, bbox_top, bbox_w, bbox_h, -1, -1, -1, -1)) # MOT format
                                # f.write(('%g ' * 10 + str(names[c]) + '\n') % (frame_idx, id, bbox_left, bbox_top, bbox_w, bbox_h, -1, -1, -1, -1)) # label format
                                # class and confidence
                                f.write(('%g ' * 10 + '\n') % (frame_idx, id, bbox_left, bbox_top, bbox_w, bbox_h, c, conf, -1, -1))  # label format
